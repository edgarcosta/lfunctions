...
Lerror_t find_zeros(Lfunc *L, uint64_t side)
{
  ...
  uint64_t n=0;
  sign_t this_sign, last_sign=sign(L->u_values_off[side][n]);
  direction_t this_dir, last_dir;
  Lerror_t ecode=ERR_SUCCESS;
  last_dir=direction(L->u_values_off[side][n], L->u_values_off[side][n+1], prec);
  if(last_dir==UNK)
  {
  ...
  while(true) {
    ...
    if(this_sign!=last_sign) // found a zero between n and n-1
    {
      ...
      if(n<L->fft_NN/OUTPUT_RATIO) {
        // need to isolate it properly
