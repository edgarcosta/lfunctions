...
// binary chop
bool isolate_zero(arb_t res, arb_t tt0, arb_t tt1, arb_t ff0, arb_t ff1, int8_t s0, int8_t s1, Lfunc *L, uint64_t side, uint64_t prec, uint64_t op_acc)
{
  static bool init=false;
  ...
  while(true)
      ...
      if(arb_is_negative(tmp3))
	{
      ...
      if(new_sign==UNK)
	{
	  if(verbose) printf("Indeterminate sign in upsampling on iteration %lu.\n",iter);
	  ...
	  if(n<L->fft_NN/OUTPUT_RATIO)// need to isolate it properly
	      ...
	      if(!newton(tmp,tmp3,L,side,prec)||!snap_point(tmp3,tmp,prec,op_acc)||!confirm_zero(tmp3,L,prec,L->zero_prec,side))
		{
		  ...
		  if(!isolate_zero(tmp,tmp1,tmp2,L->u_values_off[side][n-1],L->u_values_off[side][n],last_sign,this_sign,L,side,prec,op_acc))
		    {
		      if(verbose){printf("Setting zeros number %lu to ",count);arb_printd(tmp,20);printf("\n");}
		      ...
		      if(!confirm_zero(tmp3,L,prec,L->zero_prec,side))
			{
		      ...
		      if(verbose){printf("Setting zeros number %lu to ",count);arb_printd(tmp3,20);printf("\n");}
		...
		{
		  if(verbose){printf("setting zero number %lu to ",count);arb_printd(tmp3,200);printf("\n");}
		  arb_set(L->zeros[side][count],tmp3);
		  //arb_add_error(L->zeros[side][count++],L->zero_prec);
		  arb_add_error_2exp_si(L->zeros[side][count++],-L->target_prec-1);
		}
	  ...
	  else // don't isolate it any more, just record it
	    {
	      ...
	      if(verbose){printf("setting zero number %lu to ",count);arb_printd(L->zeros[side][count],20);printf("\n");}
	      count++;
	    }
	  last_sign=this_sign;
      ...
      if((this_dir&last_dir)==0) // change in direction
	{
	  if(((last_dir==UP)&&(this_sign==NEG))||((last_dir==DOWN)&&(this_sign==POS)))
	      ...
	      if(verbose)
		{
	      ...
	      if(verbose){printf("setting zeros %lu and %lu to ",count,count+1);
