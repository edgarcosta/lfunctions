...
// we set h=4
// and use the zeros up to height 112/r to check the list to height buthe_b=64/r
...
#include "../gp/buthe_ints.out"
  ...
  if(verbose) 
    {printf("Max allowed h is ");arb_printd(tmp,20);printf("\n");fflush(stdout);}
...
void wf1(Lfunc *L, uint64_t p, uint64_t m, uint64_t pm, arb_t bm, int64_t prec)
{
  if(arb_is_zero(bm)) // nothing to do
    return;

  arb_t s,c,tmp,tmp1,tmp2,logp,logpm;
  arb_init(s);
  arb_init(tmp);
  arb_init(tmp1);
  arb_init(tmp2);
