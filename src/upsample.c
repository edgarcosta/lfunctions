...
error_t init_upsampling(Lfunc *L)
{
  ...
  arb_set_d(L->u_H,h);
  L->u_N=H;
  if(verbose){printf("Upsampling H set to ");arb_printd(L->u_H,10);printf("\n");}
...
void do_point_f_dash(arb_ptr res, Lfunc *L, int64_t n, arb_t t, arb_t delta, arb_t sin_delta, arb_t cos_delta, arb_t A, uint64_t side, uint64_t prec)
{
