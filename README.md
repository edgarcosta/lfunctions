...
The main targets are:
 ...
 - `make check-highdeg`: run the high-degree regression suite (degree 2 to 9 L-functions: elliptic curves, their symmetric powers, genus-2 curves, classical newforms) against LMFDB and Pari golden values. Some degree >= 3 rows currently tolerate `ERR_RH_ERROR`, so those rows are regression comparisons rather than RH-certified zero/rank claims. By default it runs purely from committed base fixtures, so it needs no external toolchain. See below.
